   ...
   <div class="row mb-4 align-items-stretch">
    ...
    <div class="col-lg-6 mb-3 mb-lg-0">
        <div class="report-card h-100">
            ...
            <div class="row g-3">
                <div class="col-6">
                    <div class="metric text-primary">{{ $barangayInfo['name'] ?? 'N/A' }}</div>
                    <small class="text-muted d-block mt-1">Barangay</small>
                </div>
                <div class="col-6">
                    <div class="metric text-success">{{ number_format($barangayInfo['population'] ?? 0) }}</div>
                    <small class="text-muted d-block mt-1">Population</small>
                </div>
                <div class="col-6">
                    <div class="text-muted">{{ number_format($barangayInfo['area'] ?? 0) }} ha</div>
                    <small class="text-muted d-block mt-1">Area</small>
                </div>
                <div class="col-6">
                    <div class="text-primary">{{ $barangayInfo['registered_date'] ?? 'N/A' }}</div>
            ...
            <div class="mt-3 pt-3 border-top">
                <small class="text-success fw-bold">
                    ...
                    {{ array_sum(array_slice($evacueeBreakdown['data'] ?? [0], 0, 1)) ?? 0 }} Families Tracked
                </small>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex flex-column h-100 gap-3">
        ...
        <div class="report-card flex-grow-1 p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="metric stat-total fs-4">{{ number_format($totalHouseholds) }}</div>
            </div>
            <div class="progress" style="height: 6px;">
                <div class="progress-bar bg-primary" 
                     style="width: {{ $totalHouseholds > 0 ? round((count($evacueeDetails ?? []) / $totalHouseholds) * 100) : 0 }}%">
                </div>
            </div>
            <small class="text-muted mt-1">{{ count($evacueeDetails ?? []) }} registered</small>
        </div>
        ...
        <div class="report-card flex-grow-1 p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="metric priority-high fs-4">{{ number_format($highPriorityHouseholds) }}</div>
            </div>
            <div class="progress" style="height: 6px;">
                @php
                ...
                <div class="progress-bar bg-danger" 
                     style="width: {{ $highPriorityHouseholds > 0 ? round(($highPriorityEvacuated / $highPriorityHouseholds) * 100) : 0 }}%">
                </div>
            </div>
            <small class="text-muted mt-1">{{ $highPriorityEvacuated }} evacuated</small>
        </div>
        ...
        <div class="report-card flex-grow-1 p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="metric stat-evacuated fs-4">{{ count($evacueeDetails ?? []) }}</div>
            </div>
                        <div class="progress" style="height: 6px;">
            ...
            <small class="text-muted mt-1">{{ round((count($evacueeDetails ?? []) / $totalHouseholds) * 100, 1) }}% of total</small>
        </div>
        ...
        <div class="report-card flex-grow-1 p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="metric stat-warning fs-4">
                    ...
                    @endphp
                    {{ $totalMedicalHouseholds }}
                </div>
            </div>
                        <div class="progress" style="height: 6px;">
                <div class="progress-bar bg-warning" 
                     style="width: {{ $totalMedicalHouseholds > 0 ? round(($medicalEvacuated / $totalMedicalHouseholds) * 100) : 0 }}%">
                </div>
            </div>
            <small class="text-muted mt-1">{{ $medicalEvacuated }} / {{ $totalMedicalHouseholds }} evacuated</small>
        </div>
    </div>
</div>
</div>
...
<div class="col-lg-6 mb-4">
    <div class="report-card">
        ...
        <div id="progressContainer">
            @foreach($evacuationProgress as $purok)
            @php
            ...
            <div class="purok-item">
                <div>
                    <div class="purok-name">{{ $purok['purok'] }}</div>
                    <div class="purok-stats">{{ $purok['evacuated'] }} / {{ $purok['total'] }} evacuated 
                        <span class="badge bg-success ms-1">{{ round($percentage) }}%</span>
                    </div>
                </div>
                <div class="progress-wrapper">
                    <div class="progress-bar">
                        <div class="progress-fill" 
                             style="width: {{ $percentage }}%; background: {{ $color }} !important;">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="report-card">
                ...
                @foreach($evacueeDetails as $purokName => $details)
                <div class="border-bottom pb-3 mb-3">
                    <div class="d-flex justify-content-between align-items-center p-3 bg-light rounded" onclick="toggleDetails('{{ $purokName }}')">
                        <div>
                            <h6 class="mb-1">{{ $purokName }}</h6>
                            <small class="text-muted">{{ count($details) }} households</small>
                        </div>
                        <i class="fas fa-chevron-down details-toggle" id="toggle-{{ $purokName }}"></i>
                    </div>
                    <div class="details-panel" id="details-{{ $purokName }}">
                        @foreach($details as $detail)
                        <div class="detail-row">
                            <span><strong>{{ $detail['head_of_family'] }}</strong> ({{ $detail['category'] }})</span>
                            <span>{{ $detail['status'] }} • {{ $detail['remarks'] }}</span>
                        </div>
                        @endforeach
                    </div>
                </div>
...
const demographicsData = @json($familyDemographics);
const evacueeData = @json($evacueeBreakdown);
