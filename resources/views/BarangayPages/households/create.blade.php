...
<div style="width: 100%; padding: 0;">
    ...
    <div class="nav-pills-custom">
        ...
        <a href="{{ route('households.create') }}" class="nav-link active">
...
<div id="step1">
    <div class="row g-3">
        ...
        <div class="col-lg-6 form-group">
            ...
            <input type="text" class="form-control" id="household_name" name="household_name" required maxlength="100" value="{{ old('household_name') }}">
        </div>
        <div class="col-lg-6 form-group">
            ...
            <select class="form-select" id="purok" name="purok" required>
                ...
                @foreach($puroks as $purok)
                    <option value="{{ $purok->purok_name }}" {{ old('purok') == $purok->purok_name ? 'selected' : '' }}>
                        {{ $purok->purok_name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-lg-4 form-group">
            ...
            <input type="text" class="form-control" id="head_surname" name="head_surname" required maxlength="50" value="{{ old('head_surname') }}">
        </div>
        <div class="col-lg-4 form-group">
            <label class="form-label">First Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="head_firstname" name="head_firstname" required maxlength="50" value="{{ old('head_firstname') }}">
        </div>
        <div class="col-lg-4 form-group">
            <label class="form-label">Middle Name</label>
            <input type="text" class="form-control" id="head_middlename" name="head_middlename" maxlength="50" value="{{ old('head_middlename') }}">
        </div>
        ...
        <div class="col-lg-4 form-group">
            ...
            <div class="d-flex flex-column gap-2 mt-2">
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="gender" value="Male" id="genderMale" {{ old('gender') == 'Male' ? 'checked' : '' }} required>
                    <label class="form-check-label" for="genderMale">Male</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="gender" value="Female" id="genderFemale" {{ old('gender') == 'Female' ? 'checked' : '' }} required>
        ...
        <div class="col-lg-4 form-group">
            ...
            <div class="d-flex flex-column gap-2 mt-2">
                @foreach(['Single', 'Married', 'Widowed', 'Separated'] as $status)
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="civil_status" value="{{ $status }}" id="civil{{ str_replace(' ', '', $status) }}" {{ old('civil_status') == $status ? 'checked' : '' }} required>
                        <label class="form-check-label" for="civil{{ str_replace(' ', '', $status) }}">{{ $status }}</label>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="col-lg-4 form-group">
            ...
            <input type="number" class="form-control" id="age" name="age" min="0" max="120" required value="{{ old('age') }}">
        </div>
    </div>
...
<div id="step2" style="display: none;">
    <div class="row g-3">
        ...
        <div class="col-lg-4 form-group">
            ...
            <input type="text" class="form-control" id="barangay_name" name="barangay_name" required maxlength="100" value="{{ old('barangay_name') }}">
        </div>
        <div class="col-lg-4 form-group">
            <label class="form-label">Street <span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="street_name" name="street_name" required maxlength="100" value="{{ old('street_name') }}">
        </div>
        <div class="col-lg-4 form-group">
            <label class="form-label">House No. <span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="house_number" name="house_number" required maxlength="20" value="{{ old('house_number') }}">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12 form-group">
            ...
            <select class="form-select" id="priority_status" name="priority_status" required>
                ...
                <option value="High" {{ old('priority_status') == 'High' ? 'selected' : '' }}>High</option>
                <option value="Moderate" {{ old('priority_status') == 'Moderate' ? 'selected' : '' }}>Moderate</option>
                <option value="Low" {{ old('priority_status') == 'Low' ? 'selected' : '' }}>Low</option>
            </select>
        </div>
    </div>
                ...
                <div class="col-md-4">
                    <div class="form-check">
                        <input class="form-check-input" 
                            ...
                            value="{{ $indicator }}" 
                            id="indicator{{ str_replace(' ', '', $indicator) }}"
                            {{ in_array($indicator, old('priority_indicator', [])) ? 'checked' : '' }}>
                        <label class="form-check-label" 
                            for="indicator{{ str_replace(' ', '', $indicator) }}">
                            {{ $indicator }}
                        </label>
                    </div>
                </div>
    ...
    <div class="col-12 form-group">
        ...
        <input type="date" class="form-control" id="date_registered" name="date_registered" value="{{ old('date_registered', date('Y-m-d')) }}" required>
    </div>
...
function loadOldFamilyData() {
    @if(old('family_member_name'))
        @foreach(old('family_member_name') as $index => $name)
            setTimeout(() => {
                addFamilyMemberRow(
                    '{{ addslashes($name) }}',
                    '{{ addslashes(old("family_member_relationship.$index")) }}',
                    '{{ old("family_member_age.$index") }}'
                );
            }, 100);
        @endforeach
    @endif
